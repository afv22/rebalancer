    ...
    **Important Note:** This method operates by starting an HTTP server on the
    ...
    with callback_server():
        ...
        if automated:
            ...
            driver.get(auth_context.authorization_url)
            driver.maximize_window()

            # Log in with username and password
            username_input = driver.find_element(By.ID, 'loginIdInput')
            username_input.send_keys('avagliano')
            password_input = driver.find_element(By.ID, 'passwordInput')
            password_input.send_keys('List1-Banana-Strike')
            ...
            while True:
                now = __TIME_TIME()
                if now >= timeout_time:
                    raise ValueError("Failed to accept Terms & Conditions")
                try:
                    accept_box = driver.find_element(By.ID, 'acceptTerms')
                    break
        ...
        token_write_func = (
            __make_update_token_func(token_path)
    ...
    *Reminder:* You should never create the token file yourself or modify it in
    ...
    if __running_in_notebook():
        c = client_from_manual_flow(
            api_key, app_secret, callback_url, token_path, enforce_enums=enforce_enums
        )
        logger.info(
            "Returning client fetched using manual flow, writing" + "token to '%s'",
            token_path,
        )
    else:
